...
class MultiGethRpc {
    constructor(options) {
        ...
        this.methods.forEach((method) => {
            method.params.forEach((param, i) => this.validator.addSchema(param.schema, schema_utils_js_1.makeIdForMethodContentDescriptors(method, param)));
        });
    }
    /**
    ...
    eth_getBlockByHash(blockHash, includeTransactions) {
        ...
        const errors = lodash_1.default.chain(methodObject.params)
            .map((param, index) => {
            const isValid = this.validator.validate(schema_utils_js_1.makeIdForMethodContentDescriptors(methodObject, param), params[index]);
            const message = [
     ...
     */
    eth_getStorageAt(address, key, blockNumber) {
        const params = Array.from(arguments);
  ...
  ' + len(message) + message))).
     ...
     */
    eth_blockTransactionCountByHash(blockHash) {
        const params = Array.from(arguments);
     ...
     */
    eth_blockTransactionCountByNumber(blockNumber) {
        const params = Array.from(arguments);
